...
function canRenderReviewerAvatar(
            ...
            <div className="flex gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            {review.videos.map((videoUrl, index) => (
                                <video
                                    ...
                                    aria-label={`Video đánh giá ${index + 1}`}
                                />
                            ))}
                        </div>
                    ) : null}
                    <Button
                        ...
                        disabled={isPending}
                        aria-pressed={liked}
                        className={`mt-5 inline-flex cursor-pointer items-center gap-1.5 text-xs transition-colors disabled:cursor-wait disabled:opacity-60 ${liked ? 'font-semibold text-red-500' : 'text-zinc-400 hover:text-red-500'}`}
                    >
                        <Heart
                            className="size-4"
