...
function getPageItems(currentPage: number, totalPages: number): PageItem[] {
    ...
    for (
        let page = Math.max(1, totalPages - 1);
        page <= totalPages;
        page += 1
    ) {
        visiblePages.add(page);
...
export function ShopDirectoryPagination({
    ...
    return (
        <nav
            aria-label="Phân trang shop nội bộ"
            className="mt-8 border-t border-zinc-200 px-0 pt-6"
        >
            <div className="flex flex-col items-center justify-between gap-5 sm:flex-row">
                ...
                <div className="flex items-center gap-1.5">
                    <Button
                        ...
                        className={cn(
                            'h-9 min-w-9 rounded-lg px-2 transition-colors',
                            page <= 1
                                ? 'cursor-not-allowed border-zinc-200 bg-zinc-50 text-zinc-300 disabled:pointer-events-auto'
                                : 'cursor-pointer border-zinc-200 bg-white text-zinc-600 hover:border-zinc-950 hover:text-zinc-950',
                        )}
                    >
                        ...
                        {pageItems.map((item, index) =>
                            item === 'ellipsis' ? (
                                <span
                                    key={`ellipsis-${index}`}
                                    aria-hidden="true"
                                    className="flex h-9 min-w-9 items-center justify-center px-1 text-sm text-zinc-400"
                                >
                                    …
                                </span>
                            ) : (
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        item === page
                                            ? 'cursor-default border-zinc-950 bg-zinc-950 text-white opacity-100 hover:bg-zinc-950 hover:text-white disabled:pointer-events-auto disabled:opacity-100'
                                            : 'cursor-pointer border-zinc-200 text-zinc-600 hover:border-zinc-950 hover:text-zinc-950',
                                    )}
                                >
                                    {item}
                                </Button>
