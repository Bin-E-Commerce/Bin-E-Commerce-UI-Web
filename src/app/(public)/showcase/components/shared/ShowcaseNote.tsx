...
export function ShowcaseNote({
    ...
    return (
        <aside
            className={`${className} flex gap-3 border ${compact ? 'rounded-xl p-3' : 'rounded-2xl p-4 sm:p-5'} ${
                dark
        ...
        >
            ...
            <div>
                <p
                    className={`${compact ? 'text-xs' : 'text-sm'} font-semibold ${dark ? 'text-white' : 'text-zinc-950'}`}
                >
                    {title}
                </p>
                <div
                    className={`${compact ? 'mt-1 text-xs leading-5' : 'mt-1 text-sm leading-6'}`}
                >
                    {children}
                </div>
            </div>
