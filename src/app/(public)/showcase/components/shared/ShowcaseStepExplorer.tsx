        ...
        <section className="rounded-[1.5rem] border border-zinc-200 bg-white p-4 shadow-[0_16px_48px_-40px_rgba(24,24,27,0.45)] sm:p-5">
            <header className="flex flex-col gap-3 border-b border-zinc-100 pb-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
                <div className="min-w-0 max-w-3xl">
                    ...
                    <Heading
                        className={`${label ? 'mt-2' : ''} text-xl font-semibold leading-tight tracking-tight text-zinc-950 sm:text-2xl`}
                    >
                        {title}
                    </Heading>
                ...
                <span className="shrink-0 self-start rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 font-mono text-[10px] text-zinc-500">
                    {String(steps.length).padStart(2, '0')} bước
                </span>
            </header>
            ...
            <div
                id={detailId}
                aria-live="polite"
                className="mt-3 grid gap-3 rounded-2xl bg-zinc-50 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(230px,0.34fr)]"
            >
                <div className="min-w-0">
                    <p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">
                        ...
                        />
                        Bước {String(activeIndex + 1).padStart(2, '0')} ·{' '}
                        {activeStep.title}
                    </p>
                ...
                <aside className="min-w-0 rounded-xl border border-zinc-200 bg-white p-3.5">
                    ...
                    <p className="mt-1.5 text-sm leading-5 text-zinc-800">
                        {activeStep.output}
                    </p>
                    {activeStep.implementation ? (
                        <div className="mt-3 border-t border-zinc-100 pt-3">
                            ...
                            <code className="mt-1.5 block break-words font-mono text-[11px] leading-5 text-zinc-800">
                                {activeStep.implementation}
                            </code>
                        </div>
                    ) : null}
