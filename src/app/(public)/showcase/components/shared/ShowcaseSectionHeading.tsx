...
export function ShowcaseSectionHeading({
    ...
    return (
        <div className={width === 'full' ? 'w-full' : 'max-w-3xl'}>
            ...
            <Heading
                className={`${eyebrow ? 'mt-3' : showAccent ? 'flex items-start gap-3' : ''} ${titleSize === '2xl' ? 'text-2xl' : compact ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'} font-semibold tracking-tight text-zinc-950`}
            >
                {!eyebrow && showAccent ? <span aria-hidden="true" className="mt-2 h-6 w-1 shrink-0 rounded-full bg-zinc-950 sm:mt-2.5 sm:h-7" /> : null}
                <span>{title}</span>
            </Heading>
            <p className={`${compact ? 'mt-2 text-sm leading-6' : 'mt-3 text-sm leading-7 sm:text-base'} text-zinc-600`}>
                {description}
            </p>
        </div>
    );
