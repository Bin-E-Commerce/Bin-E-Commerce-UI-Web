...
export function ShowcaseReadingGuide({ ariaLabel, items }: ShowcaseReadingGuideProps) {
    return (
        <nav aria-label={ariaLabel}>
            <div className="flex items-center justify-between gap-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Lộ trình đọc</p>
            ...
            <ol className="mt-2 divide-y divide-zinc-100 border-t border-zinc-200">
                {items.map((item, index) => (
                    <li key={item.href}>
                        ...
                        >
                            <span className="grid size-7 shrink-0 place-items-center rounded-lg border border-zinc-200 bg-white font-mono text-[10px] text-zinc-500">{String(index + 1).padStart(2, '0')}</span>
                            <span className="min-w-0 flex-1">
                                <span className="block text-xs font-semibold text-zinc-900">{item.title}</span>
                                <span className="mt-0.5 block text-[10px] leading-4 text-zinc-500">{item.summary}</span>
                            </span>
