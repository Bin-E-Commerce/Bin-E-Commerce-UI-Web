...
export function ShowcaseDisclosure({ id, number, title, description, children }: ShowcaseDisclosureProps) {
    return (
        <details open id={id} tabIndex={-1} className="group scroll-mt-24 focus-visible:ring-2 focus-visible:ring-zinc-300 [&[open]>summary_.disclosure-chevron]:rotate-180">
            <summary className="relative grid cursor-pointer list-none grid-cols-[max-content_minmax(0,1fr)_max-content] items-start gap-3.5 py-4 [&::-webkit-details-marker]:hidden after:absolute after:inset-x-4 after:bottom-0 after:h-px after:bg-zinc-200 after:content-[''] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-zinc-400 sm:gap-4 sm:py-5">
                ...
                <span className="min-w-0 pt-0.5">
                    <span className="block text-lg font-semibold leading-snug tracking-tight text-zinc-950">{title}</span>
                    <span className="mt-1.5 block text-sm leading-6 text-zinc-800">{description}</span>
                </span>
                <ChevronDown aria-hidden="true" className="disclosure-chevron mt-1 size-4 shrink-0 text-zinc-500 transition-transform duration-200" />
            </summary>
