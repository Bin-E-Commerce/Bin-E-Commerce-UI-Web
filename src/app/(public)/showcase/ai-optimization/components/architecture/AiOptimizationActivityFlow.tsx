...
function ApprovalStep({ number, phase, title, action, reason, tradeoff }: ApprovalStepProps) {
    return (
...
function ApprovalStatus({ status, description }: { status: string; description: string }) {
    return (
        ...
        >
            <div className="rounded-2xl border border-zinc-200 bg-white p-2.5 sm:p-3">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200 px-1 pb-3">
                    <div>
                        <p className="font-mono text-[9px] font-medium uppercase tracking-[0.18em] text-zinc-500">Review → kiểm tra → quyết định → kết quả</p>
                        <p className="mt-1 text-xs font-medium text-zinc-800">Chỉ seller chọn ảnh; backend mới được phép ghi thay đổi</p>
                    </div>
                    <span className="rounded-full border border-zinc-200 bg-white px-2.5 py-1 font-mono text-[10px] text-zinc-500">4 bước</span>
                </div>
                ...
                <section className="mt-3 rounded-2xl border border-zinc-200 bg-white p-3" aria-labelledby="ai-optimization-approval-status-title">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <p className="font-mono text-[9px] font-medium uppercase tracking-[0.18em] text-zinc-500">Kết quả sau quyết định</p>
                            <h4 id="ai-optimization-approval-status-title" className="mt-1 text-xs font-semibold text-zinc-950">Mỗi trạng thái cho biết sản phẩm đang ở đâu</h4>
                        </div>
                        <span className="font-mono text-[10px] text-zinc-500">review → mutation → recovery</span>
                    </div>
                    <ul className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                        <ApprovalStatus status="REVIEW_REQUIRED" description="Seller chưa chọn output để áp dụng." />
                        <ApprovalStatus status="Version check" description="Đảm bảo không ghi đè thay đổi mới." />
                        <ApprovalStatus status="Transaction" description="Snapshot và gallery được cập nhật cùng nhau." />
                        <ApprovalStatus status="APPLIED / ROLLED_BACK" description="Có kết quả cuối và đường phục hồi." />
                    </ul>
                </section>
            </div>
        </ShowcaseDisclosure>
