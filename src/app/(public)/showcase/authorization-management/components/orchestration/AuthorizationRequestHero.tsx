...
function AuthorizationFlowStep({
    number,
    title,
    description,
    icon: Icon,
    last = false,
}: AuthorizationFlowStepProps) {
    return (
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,0.78fr)_minmax(360px,1.22fr)] lg:gap-8">
                <div className="flex h-full min-w-0 flex-col justify-center">
                    ...
                    <p className="mt-2.5 text-center text-sm leading-6 text-zinc-600">
                        Authorization không chỉ kiểm tra user đã đăng nhập. Hệ
                        thống còn phải biết user được làm gì và có được chạm vào
                        tài nguyên cụ thể đó hay không.
                    </p>
                ...
                <div className="min-w-0 rounded-xl border border-zinc-200 bg-white p-3.5 sm:p-4">
                    ...
                    <ol className="mt-4">
                        <AuthorizationFlowStep
                            number="01"
                            title="Xác thực identity"
                            icon={CircleUserRound}
                            description="Gateway xác minh JWT bằng JWKS, loại identity header giả và tạo user context đáng tin cậy."
                        />
                        <AuthorizationFlowStep
                            number="02"
                            title="Kiểm tra permission"
                            icon={KeyRound}
                            description="Permissions Guard so grant của user với quyền mà route yêu cầu; thiếu quyền thì dừng tại Gateway."
                        />
                        <AuthorizationFlowStep
                            number="03"
                            title="Resolve policy hiệu lực"
                            icon={ShieldCheck}
                            description="Auth Service hợp nhất role, assignment, permission và scope đang active thành access profile."
                        />
                        <AuthorizationFlowStep
                            number="04"
                            title="Kiểm tra resource"
                            icon={ShieldCheck}
                            description="Domain service kiểm tra ownership và scope trên shop, product hoặc resource cụ thể trước mutation."
                        />
                        <AuthorizationFlowStep
                            number="05"
                            title="Trả trải nghiệm phù hợp"
                            icon={Check}
                            last
                            description="Request hợp lệ được forward; UI dùng access profile để hiện đúng navigation, còn request sai nhận 401 hoặc 403."
                        />
                    </ol>
                    <div className="mt-1 flex items-start gap-2 border-t border-zinc-100 pt-3 text-xs leading-5 text-zinc-600">
                        ...
                        <p>
                            ...
                            frontend chỉ trình bày khả năng; quyết định bảo mật
                            luôn nằm ở Gateway và service sở hữu resource.
                        </p>
                    </div>
                </div>
            </div>
