...
export function useShippingLocations(
    ...
    const districtsQuery = useQuery({
        ...
        queryFn: () =>
            shippingLocationService.listGhnDistricts(provinceId as number),
    ...
    const wardsQuery = useQuery({
        ...
        queryFn: () =>
            shippingLocationService.listGhnWards(districtId as number),
        enabled: Boolean(districtId),
        staleTime: 60 * 60 * 1000,
    });
    const errorQuery = [provincesQuery, districtsQuery, wardsQuery].find(
        (query) => query.isError,
    );
    return {
        ...
        isLoading:
            ...
            (Boolean(provinceId) && districtsQuery.isPending) ||
            (Boolean(districtId) && wardsQuery.isPending),
        error: errorQuery
            ? 'Không tải được danh sách địa chỉ GHN. Vui lòng thử lại.'
            : null,
    };
}
