                ...
                <div className="flex min-w-0 flex-col p-5 sm:p-7">
                    ...
                    <div className="mt-6 rounded-xl border border-zinc-200 bg-zinc-950 p-5 text-white">
                        ...
                        <p className="mt-2 text-xs text-zinc-400">
                            Khoảng giá được tính từ các phân loại đang có trong
                            sản phẩm.
                        </p>
                    </div>
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
                        <HeroMetric
                            icon={Boxes}
                            label="Tồn kho"
                            value={formatSellerProductMetric(totalStock)}
                        />
                        <HeroMetric
                            icon={PackageCheck}
                            label="Đã bán"
                            value={formatSellerProductMetric(product.totalSold)}
                        />
                        <HeroMetric
                            icon={Star}
                            label="Đánh giá"
                        ...
                        <HeroMetric
                            icon={Eye}
                            label="Lượt xem"
                            value={formatSellerProductMetric(product.viewCount)}
                        />
                    </div>
                    ...
                    <div className="mt-auto grid gap-x-5 gap-y-3 border-t border-zinc-100 pt-5 sm:grid-cols-2">
                        <MetaItem
                            icon={Hash}
                            label="SKU quản lý"
                            value={product.sellerSku || 'Chưa thiết lập'}
                        />
                        <MetaItem
                            icon={CalendarClock}
                            label="Cập nhật"
                            value={formatSellerProductUpdatedAt(
                                product.updatedAt,
                            )}
                        />
                    </div>
...
function HeroMetric({ icon: Icon, label, value }: HeroMetricProps) {
    return (
        <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-3">
            ...
            <p className="mt-1.5 truncate text-sm font-bold text-zinc-950">
                {value}
            </p>
        </div>
    );
