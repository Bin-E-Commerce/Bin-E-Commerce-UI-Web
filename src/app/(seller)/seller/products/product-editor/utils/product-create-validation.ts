...
export function validateDynamicAttributes(
    ...
    for (const attribute of attributes) {
        ...
        if (!isUuid(attribute.id)) {
            ...
            errors.push(message);
            fieldErrors.push({
                fieldName: getDynamicAttributeFieldName(attribute),
                message,
            });
            continue;
        }
        ...
        if (
            ...
            !hasDynamicAttributeValue(attribute, value)
        ) {
...
export function getProductCreateStepValidations(
    ...
    return {
        basic: {
            ...
            errors: [
                ...(images.length < 2
                    ...
                    : []),
                ...(titleLength < 20
                    ? ['Tên sản phẩm cần có ít nhất 20 ký tự.']
                    : []),
                ...(!isUuid(values.categoryId)
        ...
        details: {
            ...
            errors: [
                ...(values.description.trim().length < 20
                    ? ['Mô tả sản phẩm cần có ít nhất 20 ký tự.']
                    : []),
        ...
        sales: {
            valid: validVariants,
            errors: validVariants
                ? []
                : [
                      'Mỗi phân loại cần có giá từ 100 đồng và tồn kho là số nguyên không âm.',
                  ],
        },
        shipping: {
            valid: validPackage,
            errors: validPackage
                ? []
                : ['Nhập đầy đủ cân nặng và kích thước đóng gói lớn hơn 0.'],
        },
        other: {
...
export function getFirstInvalidProductCreateStep(
    ...
    const stepIds: ProductCreateStepId[] = [
        'basic',
        'details',
        'sales',
        'shipping',
