...
const DESCRIPTION_SECTION_TEMPLATES: Record<
    DescriptionSectionId,
    { heading: string; template: string }
> = {
    highlights: {
        heading: 'Điểm nổi bật',
        template: 'Điểm nổi bật:\n- \n- \n- ',
    },
    productInfo: {
    ...
    usage: {
        heading: 'Hướng dẫn sử dụng và bảo quản',
        template:
            'Hướng dẫn sử dụng và bảo quản:\n- Cách sử dụng: \n- Cách bảo quản: ',
    },
    originWarranty: {
        heading: 'Nguồn gốc và bảo hành',
        template: 'Nguồn gốc và bảo hành:\n- Xuất xứ: \n- Bảo hành/đổi trả: ',
    },
...
function getDescriptionQuality(description: string, productName: string) {
    ...
    return {
        ...
        productInfo:
            description.trim().length >= 40 || productName.trim().length > 0,
        highlights: /điểm nổi bật|mô tả chi tiết|\n\s*[-•]/i.test(description),
        safe: !/(https?:\/\/|www\.|\bsk-|[\w.+-]+@[\w-]+\.[\w.-]+|\b(?:0|\+84)\d{8,10}\b)/i.test(
            description,
        ),
        sections: Object.fromEntries(
            Object.entries(DESCRIPTION_SECTION_TEMPLATES).map(
                ([id, section]) => [
                    id,
...
export function ProductDescriptionEditor({
    ...
    const disabledReasons = [
        ...
        readyImages.length === 0
            ? 'chưa có ảnh sản phẩm đã tải lên'
            : undefined,
    ...
    const completion = Math.min(
        100,
        Math.round((description.length / 100) * 100),
    );
    ...
    const insertSection = (sectionId: DescriptionSectionId) => {
        ...
        form.setValue(
            'description',
            `${description.trimEnd()}${separator}${section.template}`,
            {
                shouldDirty: true,
                shouldValidate: true,
            },
        ...
        <ProductFormField
            ...
            labelExtra={
                <AiAssistantButton
                    tooltip={
                        disabledReason ??
                        'AI hỗ trợ tạo bản mô tả có cấu trúc từ ảnh, ngành hàng và thông tin bạn nhập.'
                    }
            ...
            <div className="space-y-3">
                <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                    <div className="mb-3 flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-zinc-600">
                                Thêm từng mục để khách hàng đọc nhanh trên điện
                                thoại.
                            </p>
                        </div>
                        <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600">
                            {
                                Object.values(quality.sections).filter(Boolean)
                                    .length
                            }
                            /7 mục
                        </span>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                        <Button
                            ...
                            disabled={quality.sections.highlights}
                            onClick={() => insertSection('highlights')}
                        >
                            {quality.sections.highlights ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-950 text-[10px] font-semibold text-white">
                                    1
                                </span>
                            )}
                            <span>
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.productInfo}
                            onClick={() => insertSection('productInfo')}
                        >
                            {quality.sections.productInfo ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    2
                                </span>
                            )}
                            <span>
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.specifications}
                            onClick={() => insertSection('specifications')}
                        >
                            {quality.sections.specifications ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    3
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.usage}
                            onClick={() => insertSection('usage')}
                        >
                            {quality.sections.usage ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    4
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.originWarranty}
                            onClick={() => insertSection('originWarranty')}
                        >
                            {quality.sections.originWarranty ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    5
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.package}
                            onClick={() => insertSection('package')}
                        >
                            {quality.sections.package ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    6
                        ...
                        <Button
                            ...
                            variant="outline"
                            className="h-auto justify-start gap-2 px-3 py-2 text-left"
                            disabled={quality.sections.notes}
                            onClick={() => insertSection('notes')}
                        >
                            {quality.sections.notes ? (
                                <Check className="size-4 text-emerald-600" />
                            ) : (
                                <span className="flex size-5 items-center justify-center rounded-full bg-zinc-200 text-[10px] font-semibold text-zinc-700">
                                    7
                ...
                <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-zinc-50 via-white to-zinc-50 p-4">
                    <div className="mb-3 flex items-start justify-between gap-3">
                        <div>
                            <p className="text-sm font-semibold text-zinc-950">
                                Viết mô tả rõ ràng, dễ tin cậy
                            </p>
                            <p className="mt-1 text-xs leading-5 text-zinc-500">
                                Nêu điểm nổi bật, thông tin chi tiết và hướng
                                dẫn phù hợp với sản phẩm.
                            </p>
                        </div>
                    </div>
                    <div
                        className="mb-3 flex items-center gap-1 rounded-lg bg-zinc-100 p-1"
                        role="tablist"
                        aria-label="Chế độ chỉnh sửa mô tả"
                    >
                        <Button
                            type="button"
                            size="sm"
                    ...
                    {mode === 'write' ? (
                        ...
                        <div
                            id="product-description"
                            className="min-h-64 rounded-lg border border-zinc-300 bg-white p-4 text-sm leading-7 text-zinc-700"
                            aria-live="polite"
                        >
                            {description ? (
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-zinc-500">
                        <span>
                            {description.length.toLocaleString('vi-VN')}/30.000
                            ký tự
                        </span>
                        <Button
                            type="button"
                            variant="ghost"
                            size="sm"
                            className="h-8 gap-1.5 text-zinc-600 hover:text-zinc-950"
                    ...
                    >
                        <div
                            className={`h-full rounded-full transition-all ${quality.minLength ? 'bg-emerald-600' : 'bg-zinc-950'}`}
                            style={{ width: `${completion}%` }}
                        />
                    </div>
                    <p
                        id="product-description-guidance"
                        className="mt-2 text-xs leading-5 text-zinc-500"
                    >
                        Nên có ít nhất 100 ký tự để khách hàng hiểu rõ sản phẩm.
                    </p>
                </div>

                <div
                    className="grid gap-2 rounded-xl border border-dashed border-zinc-300 bg-white p-4 text-xs sm:grid-cols-2"
                    aria-label="Kiểm tra chất lượng mô tả"
                >
                    {[
                        [quality.minLength, 'Đủ độ dài tối thiểu 100 ký tự'],
                        [
                            quality.productInfo,
                            'Có thông tin nhận diện sản phẩm',
                        ],
                        [
                            quality.highlights,
                            'Có điểm nổi bật hoặc cấu trúc rõ ràng',
                        ],
                ...
                <AlertDialogContent className="max-w-3xl overflow-hidden p-0">
                    <div className="border-b border-zinc-200 bg-white px-6 py-5">
                        <AlertDialogHeader className="text-left">
                            <div className="flex items-start gap-3">
                                <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-zinc-950 text-white shadow-sm">
                                    <AiAssistantIcon
                                        size={22}
                                        className="invert"
                                    />
                    ...
                    <div className="max-h-[min(72vh,44rem)] space-y-4 overflow-y-auto p-6">
                        <div
                            ...
                            aria-label="Ngữ cảnh AI đang sử dụng"
                        >
                            <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-zinc-50 to-white p-3.5 shadow-sm">
                                <div className="mb-3 flex size-8 items-center justify-center rounded-lg bg-zinc-950 text-white">
                                ...
                                <p className="mt-1 line-clamp-2 text-sm font-semibold leading-5 text-zinc-950">
                                    {references.category?.name ?? 'Chưa chọn'}
                                </p>
                            </div>
                            <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-zinc-50 to-white p-3.5 shadow-sm">
                                <div className="mb-3 flex size-8 items-center justify-center rounded-lg bg-zinc-100 text-zinc-950">
                        ...
                        <div className="flex items-start gap-2 rounded-xl border border-zinc-200 bg-white p-4 text-sm leading-6 text-zinc-600">
                            ...
                            <p>
                                AI chỉ gợi ý. Hãy kiểm tra thông tin, thông số
                                và hướng dẫn trước khi lưu sản phẩm.
                            </p>
                        </div>
                        <Button
                            type="button"
                            ...
                            disabled={
                                Boolean(disabledReason) || assistant.isLoading
                            }
                        ...
                        <p className="text-xs text-zinc-500" aria-live="polite">
                            {disabledReason ??
                                `${readyImages.length}/3 ảnh sản phẩm và thông tin ngành hàng sẽ được dùng để tạo nội dung.`}
                        </p>
                        ...
                        {assistant.isLoading ? (
                            <div
                                className="space-y-3"
                                aria-live="polite"
                                aria-label="AI đang tạo mô tả sản phẩm"
                            >
                                <Skeleton className="h-56 w-full rounded-xl" />
                                <Skeleton className="h-5 w-2/3 rounded" />
                            </div>
                        ) : null}
                        {!assistant.isLoading && assistant.description ? (
                            <div
                            ...
                            >
                                ...
                                <div className="max-h-80 overflow-y-auto whitespace-pre-wrap text-sm leading-7 text-zinc-700">
                                    {assistant.description}
                                </div>
                            </div>
                        ) : null}
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <p className="font-semibold">
                                            AI đã lọc thông tin cần bảo vệ
                                        </p>
                                        <ul className="mt-1 list-disc space-y-1 pl-4">
                                            {assistant.warnings.map(
                                                (warning) => (
                                                    <li
                    ...
                    <AlertDialogFooter className="border-t border-zinc-200 bg-zinc-50 px-6 py-4">
                        ...
                        <Button
                            ...
                            disabled={
                                !assistant.description || assistant.isLoading
                            }
