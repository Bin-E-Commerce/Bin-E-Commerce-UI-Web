...
export function ProductContentAssistant({
    ...
    const readyImages = useMemo(
        () => getReadyProductImages(values.images),
        [values.images],
    );
    ...
    const disabledReasons = [
        ...
        readyImages.length === 0
            ? 'chưa có ít nhất một ảnh sản phẩm đã tải lên'
            : undefined,
    ...
    return (
        <>
            <AiAssistantButton
                tooltip={
                    disabledReason ??
                    'AI hỗ trợ tạo tên sản phẩm từ ảnh, ngành hàng và thông tin bạn nhập.'
                }
                ...
                <AlertDialogContent className="max-w-2xl overflow-hidden p-0">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <AlertDialogDescription className="mt-1 text-sm leading-5 text-zinc-600">
                                        Phân tích tối đa 3 ảnh, ngành hàng và
                                        thông tin bạn nhập để đề xuất tên phù
                                        hợp.
                                    </AlertDialogDescription>
                                </div>
                            </div>
                    ...
                    <div className="max-h-[min(70vh,38rem)] space-y-4 overflow-y-auto p-6">
                        <div className="flex items-start gap-2 rounded-xl bg-zinc-50 p-4 text-sm leading-6 text-zinc-600">
                            ...
                            <p>
                                AI chỉ gợi ý. Bạn cần chọn tên và kiểm tra lại
                        ...
                        >
                            {disabledReason ??
                                `${readyImages.length}/3 ảnh sản phẩm đã sẵn sàng cho AI.`}
                        </p>
                        ...
                        {assistant.isLoading ? (
                            <div
                                className="space-y-3"
                                aria-live="polite"
                                aria-label="AI đang tạo tên sản phẩm"
                            >
                                {[1, 2, 3].map((item) => (
                                    <Skeleton
                                        key={item}
                                    ...
                                    >
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div className="min-w-0">
                                                ...
                                                <p
                                                    className={`mt-1 text-xs leading-5 ${suggestion.recommended ? 'text-zinc-300' : 'text-zinc-500'}`}
                                                >
                                                    {suggestion.reason}
                                                </p>
                                            </div>
