            ...
            <div className="space-y-6">
                ...
                <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4">
                    <p className="text-sm font-semibold text-zinc-950">
                        Điều kiện mở kênh vận chuyển
                    </p>
                    <div className="mt-4 grid gap-4 sm:grid-cols-3">
                        <ShippingReadiness
                            icon={Box}
                            title="Kiện hàng"
                            description="Cân nặng và kích thước thực tế sau khi đóng gói."
                        />
                        <ShippingReadiness
                            icon={Ruler}
                            title="Khả năng nhận hàng"
                            description="Đối chiếu giới hạn kích thước, khối lượng của từng đối tác."
                        />
                        <ShippingReadiness
                            icon={Truck}
                            title="Phương án giao"
                            description="Ước tính phí và thời gian từ địa chỉ lấy hàng mặc định của shop."
                        />
                    </div>
                    <p className="mt-4 border-t border-zinc-200 pt-3 text-xs leading-5 text-zinc-500">
                        Danh sách hãng giao hàng, phí và thời gian dự kiến sẽ
                        chỉ hiển thị khi Shipping Service trả về phương án phù
                        hợp.
                    </p>
                </div>
            </div>
        ...
        <ProductFormField label={label} required error={error}>
            <div className="flex">
                <Input
                    ...
                    {...form.register(`package.${name}`)}
                />
                <span className="flex h-11 min-w-12 items-center justify-center rounded-r-md border border-l-0 border-zinc-200 bg-zinc-50 px-3 text-xs text-zinc-500">
                    {unit}
                </span>
            </div>
        </ProductFormField>
