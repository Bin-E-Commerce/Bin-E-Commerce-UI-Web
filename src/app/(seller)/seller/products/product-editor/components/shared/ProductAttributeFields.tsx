...
export function ProductAttributeFields({
    ...
    if (visibleAttributes.length === 0) {
        return (
            <div className="rounded-md border border-dashed border-zinc-300 px-4 py-6 text-sm text-zinc-500">
                Chọn ngành hàng để hiển thị thông số sản phẩm phù hợp.
            </div>
        );
    }
...
function ProductAttributeField({
    ...
    const value = form.watch(`attributes.${attribute.id}`) ?? {
        selectedOptionIds: [],
        valueText: '',
        valueNumber: '',
        valueBoolean: null,
    ...
    const error =
        ...
        fieldErrors?.valueText?.message ||
        fieldErrors?.valueNumber?.message ||
        fieldErrors?.valueBoolean?.message;
    const fieldId = `product-attribute-${attribute.id}`;
    const isOptionField =
        attribute.inputType === 'SINGLE_SELECT' ||
    ...
    return (
        <ProductFormField
            ...
            hint={
                isOptionField && attribute.maxSelections
                    ? `Chọn tối đa ${attribute.maxSelections} giá trị.`
                    : undefined
            }
        >
...
function ProductAttributeInput({
    ...
    if (isOptionField) {
        ...
        const selectedValidOptionIds = value.selectedOptionIds.filter(
            (optionId) => isUuid(optionId),
        );

        return (
            <ProductAttributeOptionSelect
                value={selectedValidOptionIds}
                options={validOptions}
                multiple={attribute.inputType === 'MULTI_SELECT'}
                maxSelections={attribute.maxSelections}
                placeholder={`Chọn ${attribute.displayName.toLocaleLowerCase('vi-VN')}`}
                onChange={(selectedOptionIds) =>
                    setSelectedOptions(form, attribute.id, selectedOptionIds)
                }
            />
        );
    }
    if (attribute.inputType === 'BOOLEAN') {
        return (
            <div className="flex h-11 items-center justify-between rounded-md border border-zinc-200 px-3">
                ...
                <Switch
                    id={fieldId}
                    checked={value.valueBoolean === true}
                    onCheckedChange={(checked) =>
                        form.setValue(
                            `attributes.${attribute.id}.valueBoolean`,
                            checked,
    ...
    if (attribute.inputType === 'TEXTAREA') {
        return (
            <Textarea
                ...
                onChange={(event) =>
                    form.setValue(
                        `attributes.${attribute.id}.valueText`,
                        event.target.value,
                        { shouldDirty: true, shouldValidate: true },
                    )
                }
    ...
    return (
        <Input
            id={fieldId}
            type={getInputType(attribute.inputType)}
            step={attribute.inputType === 'DECIMAL' ? 'any' : undefined}
            value={
            ...
            onChange={(event) =>
                setTextAttributeValue(form, attribute, event.target.value)
            }
            className="h-11"
        />
    );
...
function setSelectedOptions(
    ...
    form.setValue(
        `attributes.${attributeId}.selectedOptionIds`,
        selectedOptionIds.filter((optionId) => isUuid(optionId)),
        { shouldDirty: true, shouldValidate: true },
    );
}
...
function setTextAttributeValue(
    ...
    const valueField =
        attribute.inputType === 'INTEGER' || attribute.inputType === 'DECIMAL'
            ? 'valueNumber'
            : 'valueText';
    form.setValue(`attributes.${attribute.id}.${valueField}`, value, {
        shouldDirty: true,
        shouldValidate: true,
    });
}
