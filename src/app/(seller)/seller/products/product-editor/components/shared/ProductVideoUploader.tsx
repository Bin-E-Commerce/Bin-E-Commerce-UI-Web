    ...
    return (
        <div className="space-y-3">
            <input
                ...
                className="hidden"
                onChange={(event) => void upload.selectVideo(event)}
            />
            {video ? (
                <div className="flex flex-col gap-3 rounded-md border border-zinc-200 bg-zinc-50 p-3 sm:flex-row sm:items-center">
                    <video
                        src={video.previewUrl || video.publicUrl}
                        controls
                        className="aspect-video w-full max-w-56 rounded-md bg-zinc-950 object-cover"
                    />
                ...
                <Button
                    ...
                    className={cn(
                        'flex min-h-28 w-full flex-col items-center justify-center gap-2 rounded-md border border-dashed border-zinc-300 bg-zinc-50 text-sm text-zinc-600 transition-colors',
                        'hover:border-zinc-500 hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-60',
                    )}
            ...
            <p className="flex items-center gap-1 text-xs text-zinc-500">
                ...
                MP4 hoặc WebM, dài 10-60 giây, tối đa 30 MB.
            </p>
        </div>
    );
}
