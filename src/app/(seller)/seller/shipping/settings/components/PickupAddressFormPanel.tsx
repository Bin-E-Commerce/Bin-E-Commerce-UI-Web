...
} from '../../../register/components/shared/SellerCombobox';
...
const EMPTY_ADDRESS: PickupAddressFormValues = {
    contactName: '',
    phone: '',
    provinceId: '',
    provinceName: '',
...
export function PickupAddressFormPanel({
    editingAddress,
    isPending,
    onCancelEdit,
    onSubmit,
}: PickupAddressFormPanelProps) {
    const [values, setValues] = useState<PickupAddressFormValues>(() =>
        getInitialAddress(editingAddress),
    );
    ...
    const districtOptions = useMemo<SellerComboboxOption[]>(
        () =>
            districts.map((item) => ({
                value: String(item.id),
                label: item.name,
    ...
    const updateValues = (patch: Partial<PickupAddressFormValues>) =>
    ...
    return (
        <section
            ...
            aria-labelledby="pickup-address-form-title"
        >
            <div className="flex items-start gap-3">
                ...
                <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-400">
                        {editingAddress ? 'Cập nhật kho' : 'Thêm kho mới'}
                    </p>
                    <h2
                        id="pickup-address-form-title"
                    ...
                    <p className="mt-1 text-sm leading-6 text-zinc-500">
                        Chọn địa chỉ từ danh sách GHN để tính phí và tạo vận đơn
                        chính xác.
                    </p>
                </div>
            </div>
            <div className="mt-6 space-y-4">
                ...
                <Field label="Quận / Huyện" htmlFor="pickup-district" required>
                    <SellerCombobox
                        id="pickup-district"
                        value={values.districtId}
                        options={districtOptions}
                        placeholder={
                            provinceId ? 'Chọn quận / huyện' : 'Chọn tỉnh trước'
                        }
                ...
                <Field label="Phường / Xã" htmlFor="pickup-ward" required>
                    <SellerCombobox
                        id="pickup-ward"
                        ...
                        loading={
                            Boolean(districtId) && isLoading && !wards.length
                        }
                        disabled={!districtId}
                ...
                >
                    <input
                        ...
                        onChange={(event) =>
                            updateValues({ addressLine: event.target.value })
                        }
