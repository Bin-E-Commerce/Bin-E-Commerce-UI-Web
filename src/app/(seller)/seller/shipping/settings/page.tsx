...
import {
    ...
    type PickupAddressFormValues,
} from './components/PickupAddressFormPanel';
...
export default function ShippingSettingsPage() {
    ...
    const defaultAddress =
        current?.pickupAddresses.find(
            (address) => address.id === current.settings.defaultPickupAddressId,
    ...
    const addressMutation = useMutation({
        mutationFn: ({
            ...
            id: string | null;
            values: PickupAddressFormValues;
        }) => {
            const payload = {
        ...
        onSuccess: (data, variables) => {
            ...
            toast.success(
                variables.id
                    ? 'Đã cập nhật địa chỉ lấy hàng.'
                    : 'Đã thêm địa chỉ lấy hàng.',
            );
        },
        onError: () =>
            toast.error(
                'Vui lòng chọn đầy đủ địa chỉ và kiểm tra thông tin liên hệ.',
            ),
    });
        ...
        return (
            <section className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center">
                ...
                <h1 className="mt-3 font-semibold text-red-950">
                    Không thể tải thiết lập giao nhận
                </h1>
    ...
    const handleEditAddress = (address: PickupAddress) => {
        ...
        window.requestAnimationFrame(() => {
            document
                .getElementById('pickup-address-form')
                ...
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
                            Quản lý nơi lấy hàng của shop để đơn được tạo vận
                            đơn chính xác.
                        </p>
                    </div>
                        ...
                        <div className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-zinc-950">
                                    {defaultAddressReady
                                        ? 'Sẵn sàng nhận đơn'
                                        : 'Cần đối chiếu địa chỉ'}
                                </p>
                                <p className="mt-0.5 text-xs text-zinc-500">
                                    {defaultAddressReady
                                        ? 'Địa chỉ lấy hàng đã được chọn'
                                        : 'Chọn khu vực GHN cho kho mặc định'}
                                </p>
                            </div>
                        </div>
                        <Link
                            href="/seller/shipping/providers"
                            className="inline-flex h-[58px] cursor-pointer items-center gap-2 rounded-xl bg-zinc-950 px-4 text-sm font-semibold text-white transition hover:bg-zinc-800"
                        >
            ...
            <main className="min-w-0 space-y-6">
                <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm leading-6 text-zinc-500">
                                    Địa chỉ này sẽ được dùng khi tạo báo giá và
                                    vận đơn.
                                </p>
                            </div>
                        </div>
                        {defaultAddress ? (
                            <Button
                                variant="ghost"
                                type="button"
                                className="inline-flex h-9 cursor-pointer items-center gap-1.5 self-start rounded-lg border border-zinc-200 px-3 text-xs font-semibold text-zinc-700 transition hover:border-zinc-950 hover:text-zinc-950"
                                onClick={() =>
                                    handleEditAddress(defaultAddress)
                                }
                            >
                    ...
                    {defaultAddress ? (
                        <div className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-50/70 p-5 sm:p-6">
                            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 pb-4">
                                ...
                                <span className="text-xs text-zinc-500">
                                    {defaultAddressReady
                                        ? 'Dùng cho báo giá và vận đơn'
                                        : 'Chưa đủ mã khu vực GHN'}
                                </span>
                            </div>
                            <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(180px,0.7fr)_minmax(180px,0.7fr)_minmax(280px,1.6fr)]">
                                ...
                                <div>
                                    ...
                                    <p className="mt-2 font-medium leading-6 text-zinc-900">
                        ...
                        <div className="mt-6 rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-5 text-sm leading-6 text-zinc-600">
                            Chưa có địa chỉ lấy hàng. Tạo địa chỉ đầu tiên ở khu
                            vực bên phải để bắt đầu nhận đơn.
                        </div>
                    )}
                </section>
