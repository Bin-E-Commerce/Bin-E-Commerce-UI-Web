        ...
        <div className="mt-3">
            <div className="flex flex-wrap gap-2">
                {existingImages.map((imageUrl, index) => (
                    <div
                        key={`existing-image-${imageUrl}`}
                        className="relative size-16 overflow-hidden rounded-lg border border-zinc-200 bg-white"
                    >
                        <Image
                            src={imageUrl}
                            alt={`Ảnh review cũ ${index + 1}`}
                            fill
                            sizes="64px"
                        ...
                        <Button
                            ...
                            disabled={disabled}
                            className="absolute right-1 top-1 flex size-5 items-center justify-center rounded-full bg-zinc-950/80 text-white disabled:opacity-50"
                        >
                ...
                {existingVideos.map((videoUrl, index) => (
                    <div
                        key={`existing-video-${videoUrl}`}
                        className="relative size-16 overflow-hidden rounded-lg border border-zinc-200 bg-zinc-950"
                    >
                        <video
                            src={videoUrl}
                            muted
            ...
            <p className="mt-2 text-[11px] text-zinc-400">
                Toi da 5 anh (5MB/anh) va 1 video MP4/WebM (30MB)
            </p>
        </div>
    );
}
