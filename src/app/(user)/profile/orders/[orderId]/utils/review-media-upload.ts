...
function buildProcessedReviewImageUrl(
    publicBaseUrl: string | null,
    objectKey: string,
): string {
    if (!publicBaseUrl) throw new Error('Media CDN chua duoc cau hinh.');
    const [, , purpose, ownerId, assetId] = objectKey.split('/');
    if (purpose !== 'review_image' || !ownerId || !assetId)
        throw new Error('Duong dan anh review khong hop le.');
...
function buildOriginalReviewVideoUrl(
    publicBaseUrl: string | null,
    objectKey: string,
): string {
    if (!publicBaseUrl) throw new Error('Media CDN chua duoc cau hinh.');
    const [, , purpose, ownerId, assetId] = objectKey.split('/');
    if (purpose !== 'review_video' || !ownerId || !assetId)
        throw new Error('Duong dan video review khong hop le.');
...
export async function uploadReviewMedia(
    ...
    const results = await Promise.allSettled(
        uploadTasks.map(async ({ file, purpose }) => {
            ...
            const uploadedAsset = {
                ...
                purpose,
            } satisfies ReviewMediaCleanupAsset;
            uploadedAssets.push(uploadedAsset);
            return {
                purpose,
                url:
                    purpose === 'review_image'
                        ? buildProcessedReviewImageUrl(
                              presigned.publicBaseUrl,
                              presigned.objectKey,
                          )
                        : buildOriginalReviewVideoUrl(
                              presigned.publicBaseUrl,
                              presigned.objectKey,
                          ),
            };
    ...
    if (failedUpload) {
        ...
        throw failedUpload.reason instanceof Error
            ? failedUpload.reason
            : new Error('Không thể upload media đánh giá.');
    }
    ...
    return {
        imageUrls: successfulResults
            .filter((result) => result.purpose === 'review_image')
            .map((result) => result.url),
        videoUrls: successfulResults
            .filter((result) => result.purpose === 'review_video')
            .map((result) => result.url),
        uploadedAssets,
    };
}
