...
export function ActivityRhythmCard({ overview }: Props) {
    ...
    const activeDays = daily.filter(
        (item) => item.events > 0 || item.purchaseCompleted > 0 || item.purchaseReturned > 0,
    ).length;
    const peakDay = daily.length
        ? daily.reduce((current, item) => (item.events > current.events ? item : current))
        : null;
    const averageEvents = dailyTimeline.length
        ? Math.round(dailyTimeline.reduce((sum, item) => sum + item.events, 0) / dailyTimeline.length)
        : 0;
        ...
        <section className="grid min-w-0 gap-4">
            <div className="min-w-0 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <h2 className="text-base font-semibold text-zinc-950">Nhịp hoạt động</h2>
                                <p className="mt-0.5 text-xs text-zinc-400">Theo từng ngày</p>
                            </div>
                        </div>
                        <p className="mt-4 text-sm text-zinc-500">
                            Toàn bộ event tương tác trong khoảng thời gian đã chọn.
                        </p>
                    </div>
                ...
                <div className="mt-5 grid gap-2 sm:grid-cols-3">
                    <SummaryTile label="Ngày có dữ liệu" value={`${activeDays}/${dailyTimeline.length}`} />
                    <SummaryTile
                ...
                <DailyPerformanceChart daily={dailyTimeline} activeDays={activeDays} />
            </div>
        </section>
    );
}
