    ...
    return (
        <div className="mt-5">
            ...
            <div
                ...
                style={{ scrollbarColor: '#18181b #f4f4f5', scrollbarWidth: 'thin' }}
            >
                            ...
                            >
                                <div className="flex items-center justify-between gap-3 border-b border-zinc-100 pb-3">
                                    ...
                                    <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-semibold text-zinc-700">
                                        {item.events.toLocaleString('vi-VN')} tổng event
                                    </span>
                                </div>
                                <div className="mt-3 grid grid-cols-2 gap-2">
                                    <MobileDailyMetric label="Xem sản phẩm" value={item.productViews} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Impression" value={item.impressions} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Click" value={item.clicks} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Tìm kiếm" value={item.searches} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Thêm giỏ" value={item.cartAdds} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Xóa giỏ" value={item.cartRemovals} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Mua thành công" value={item.purchaseCompleted} maxEvents={maxEventTypeCount} />
                                    <MobileDailyMetric label="Trả hàng" value={item.purchaseReturned} maxEvents={maxEventTypeCount} />
                                </div>
                            </article>
                        ))}
                    </div>
                        ...
                        <div className="divide-y divide-zinc-100">
                            {daily.map((item) => (
                                ...
                                >
                                    ...
                                    <DailyMetricCell label="tổng" value={item.events} maxEvents={maxTotalEvents} />
                                    <DailyMetricCell label="xem" value={item.productViews} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="hiện" value={item.impressions} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="click" value={item.clicks} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="tìm" value={item.searches} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="thêm" value={item.cartAdds} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="xóa" value={item.cartRemovals} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="mua" value={item.purchaseCompleted} maxEvents={maxEventTypeCount} />
                                    <DailyMetricCell label="trả" value={item.purchaseReturned} maxEvents={maxEventTypeCount} />
                                </div>
                            ))}
                        </div>
                    </div>
...
function isEmptyDay(item: DailyActivity): boolean {
    return item.events === 0 && item.purchaseCompleted === 0 && item.purchaseReturned === 0;
}
...
function DailyMetricCell({ label, value, maxEvents }: MetricProps) {
    const width = value > 0 ? `${Math.max(8, (value / maxEvents) * 100)}%` : '0%';

    return (
        <div className="min-w-[5.5rem]">
            <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] text-zinc-400">{label}</span>
                <span className="text-xs font-semibold text-zinc-800">{value.toLocaleString('vi-VN')}</span>
            </div>
    ...
    return (
        <div className="rounded-lg bg-zinc-50 px-2.5 py-2">
            <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs font-semibold text-zinc-900">{value.toLocaleString('vi-VN')}</span>
            </div>
