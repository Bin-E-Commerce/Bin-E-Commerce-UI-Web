...
export function OverviewMetrics({ overview }: Props) {
    ...
    const summaryCards = [
        {
            ...
            value: (totals?.totalInteractions ?? totals?.events ?? 0).toLocaleString('vi-VN'),
            icon: Activity,
        },
        {
    ...
    const behaviorMetrics = [
        { label: 'Xem sản phẩm', value: eventBreakdown.productViews, icon: Eye },
        { label: 'Impression', value: eventBreakdown.impressions, icon: Radio },
        { label: 'Click', value: eventBreakdown.clicks, icon: MousePointerClick },
        { label: 'Tìm kiếm', value: eventBreakdown.searches, icon: Search },
        { label: 'Thêm vào giỏ', value: eventBreakdown.cartAdds, icon: ShoppingCart },
        { label: 'Xóa khỏi giỏ', value: eventBreakdown.cartRemovals, icon: Trash2 },
        { label: 'Mua thành công', value: eventBreakdown.purchaseCompleted, icon: PackageCheck },
        { label: 'Trả hàng', value: eventBreakdown.purchaseReturned, icon: RotateCcw },
    ];
    ...
    return (
        <>
            <section className="space-y-3">
                <div>
                    <h2 className="text-base font-semibold text-zinc-950">Tổng quan</h2>
                        ...
                        <div key={label} className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
                            <div className="flex items-start justify-between gap-3 text-zinc-500">
                                <div>
                                    ...
                                    <p className="mt-1 text-xs leading-4 text-zinc-400">{description}</p>
                                </div>
            ...
            <section className="space-y-3">
                <div className="flex flex-wrap items-end justify-between gap-2">
                    <div>
                        <h2 className="text-base font-semibold text-zinc-950">Hành vi theo loại</h2>
                        <p className="mt-1 text-sm text-zinc-500">
                            Tất cả tín hiệu người dùng phát sinh trong khoảng thời gian đã chọn.
                        </p>
                    </div>
                    <span className="text-xs text-zinc-400">8 loại hành vi</span>
                </div>
                <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                    {behaviorMetrics.map(({ label, value, icon: Icon }) => (
                        <div key={label} className="flex items-center justify-between gap-3 rounded-lg border border-zinc-200 bg-white px-4 py-3">
