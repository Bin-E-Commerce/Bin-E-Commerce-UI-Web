...
export function getAddresses() {
    return authorizedAxios
        .get<ApiResponse<UserAddress[]>>(ADDRESSES)
...
export function createAddress(payload: CreateAddressPayload) {
    return authorizedAxios
        .post<ApiResponse<UserAddress>>(ADDRESSES, payload)
        .then((response) => response.data);
}
...
export function updateAddress(id: string, payload: CreateAddressPayload) {
    return authorizedAxios
        .put<ApiResponse<UserAddress>>(`${ADDRESSES}/${id}`, payload)
        .then((response) => response.data);
}
...
export function deleteAddress(id: string) {
    return authorizedAxios
        .delete<ApiResponse<null>>(`${ADDRESSES}/${id}`)
        .then((response) => response.data);
}
