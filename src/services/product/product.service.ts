...
export const productService = {
    ...
    getOrderReviewStatus: (orderId: string) =>
        authorizedAxios
            .get<OrderReviewStatusResponse>(`${API_VERSION}/products/reviews/me`, { params: { orderId } })
            .then((response) => response.data),

    // Gửi review đầy đủ cho đúng order item; Product Service tự kiểm tra purchase proof trước khi lưu.
    createReview: (productId: string, input: { orderItemId: string; rating: number; title?: string; content?: string; images?: string[]; videos?: string[]; isAnonymous?: boolean }) =>
        authorizedAxios
    ...
    updateReview: (reviewId: string, input: { rating?: number; title?: string; content?: string; images?: string[]; videos?: string[]; isAnonymous?: boolean }) =>
        authorizedAxios
    ...
    likeReview: (reviewId: string) =>
        authorizedAxios
            .put<{ reviewId: string; liked: boolean; likeCount: number }>(`${API_VERSION}/products/reviews/${reviewId}/like`)
            .then((response) => response.data),

    // Bỏ like idempotent và đồng bộ lại trạng thái của đúng review đang tương tác.
    unlikeReview: (reviewId: string) =>
        authorizedAxios
            .delete<{ reviewId: string; liked: boolean; likeCount: number }>(`${API_VERSION}/products/reviews/${reviewId}/like`)
            .then((response) => response.data),
};
