...
function normalizeRoutePoint(
    point: ShipmentRoutePoint,
): ShipmentRoutePoint | null {
...
function buildDisplayRoute(
    ...
    const normalizedRoute = (Array.isArray(routePoints) ? routePoints : []).map(
        normalizeRoutePoint,
    );
    if (
        normalizedRoute.length >= 2 &&
        normalizedRoute.every((point) => point !== null)
    ) {
        return normalizedRoute.filter(
            (point): point is ShipmentRoutePoint => point !== null,
        );
    }
    return [
        {
...
function findCurrentIndex(
    ...
    return points.reduce((nearestIndex, point, index) => {
        ...
        const nearestDistance =
            (nearest.latitude - currentLocation.latitude) ** 2 +
            (nearest.longitude - currentLocation.longitude) ** 2;
        const pointDistance =
            (point.latitude - currentLocation.latitude) ** 2 +
            (point.longitude - currentLocation.longitude) ** 2;
...
export default function ShipmentMap({
    ...
    const upcomingRoute = displayRoute.slice(
    ...
    return (
        <div className="relative isolate z-0 h-[320px] overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-100 shadow-sm sm:h-[360px]">
            <MapContainer
                center={center}
                zoom={13}
            ...
            >
                ...
                <Polyline
                    positions={displayRoute.map(toPosition)}
                    pathOptions={{
                        color: '#a1a1aa',
                        weight: 5,
                        opacity: 0.72,
                ...
                {upcomingRoute.length > 1 ? (
                    <Polyline
                        positions={upcomingRoute.map(toPosition)}
                        pathOptions={{
                            color: '#d4d4d8',
                            weight: 5,
                            opacity: 0.9,
                ...
                {completedRoute.length > 1 ? (
                    <Polyline
                        positions={completedRoute.map(toPosition)}
                        pathOptions={{
                            color: '#18181b',
                            weight: 6,
                            opacity: 0.9,
                        ...
                        <CircleMarker
                            ...
                            radius={
                                isCurrent
                                    ? 10
                                    : index === 0 ||
                                        index === displayRoute.length - 1
                                      ? 7
                                      : 5
                            }
                            pathOptions={{
                                color: isCurrent ? '#059669' : '#52525b',
                                fillColor: isCurrent
                                    ? '#10b981'
                                    : isCompleted
                                      ? '#18181b'
            ...
            <div className="pointer-events-none absolute inset-x-3 top-3 z-[500] flex items-start justify-between gap-2">
                <div className="max-w-[72%] rounded-xl border border-zinc-200/90 bg-white/95 px-3 py-2 shadow-lg backdrop-blur">
                    ...
                    <p className="mt-0.5 truncate text-xs font-semibold text-zinc-950">
                        {normalizedCurrentLocation.label}
                    </p>
                </div>
            ...
            <div className="pointer-events-none absolute bottom-3 left-3 z-[500] flex items-center gap-3 rounded-lg border border-zinc-200/90 bg-white/95 px-3 py-2 text-[10px] font-medium text-zinc-600 shadow-lg backdrop-blur">
                ...
                <span className="flex items-center gap-1.5">
                    <span className="h-1.5 w-5 rounded-full border-t-2 border-dashed border-zinc-400" />
                    Sắp đi
                </span>
            </div>
        </div>
