...
// Hook sở hữu query key, cache invalidation và thông báo thao tác; API provider nằm ở shipping service.
...
const shipmentKey = (role: 'seller' | 'customer', orderId: string) =>
    ['shipment', role, orderId] as const;
...
export function useCustomerTracking(orderId: string, enabled = true) {
    return useQuery({
        ...
        refetchInterval: (query) => {
            ...
            const active = shipments.some((shipment) =>
                [
                    'READY_TO_SHIP',
                    'PICKUP_ASSIGNED',
                    'PICKED_UP',
...
function useSellerShipmentMutation<T, TVariables = void>(
    orderId: string,
...
export function useCreateSellerShipment(orderId: string) {
    return useSellerShipmentMutation(
        orderId,
        () => createSellerShipment(orderId),
        'Shop đã chuẩn bị hàng và tạo vận đơn thành công.',
    );
}
...
export function useRefreshSellerShipment(orderId: string) {
    return useSellerShipmentMutation(
        orderId,
        () => refreshSellerShipment(orderId),
        'Đã làm mới trạng thái vận chuyển.',
    );
}
...
export function useCancelSellerShipment(orderId: string) {
    return useSellerShipmentMutation(
        orderId,
        (reason: string) => cancelSellerShipment(orderId, reason),
        'Đã hủy vận đơn và hoàn lại tồn kho.',
    );
}
...
export function useAdvanceDemoSellerShipment(orderId: string) {
    return useSellerShipmentMutation(
        orderId,
        () => advanceDemoSellerShipment(orderId),
        'Đã chuyển sang chặng tiếp theo.',
    );
}
...
export function useAdvanceDemoCustomerReturnShipment(orderId: string) {
    ...
    return useMutation({
        mutationFn: (returnId: string) =>
            advanceDemoCustomerReturnShipment(returnId),
        onSuccess: async (shipment) => {
            queryClient.setQueryData(
                shipmentKey('customer', orderId),
                (current: { shipments: (typeof shipment)[] } | undefined) => {
            ...
            toast.success(
                shipment.status === 'RETURNED'
                    ? 'Hàng hoàn đã về shop.'
                    : 'Đã chuyển sang chặng hoàn tiếp theo.',
            );
        },
        onError: (error) => toast.error(getErrorMessage(error)),
    });
...
export function usePrintSellerShipmentLabel(orderId: string) {
    return useMutation({
        ...
        onSuccess: (blob) => {
            ...
            anchor.href = url;
            const extension = blob.type.toLowerCase().includes('html')
                ? 'html'
                : 'pdf';
            ...
            toast.success(
                extension === 'html'
                    ? 'Đã tải nhãn vận đơn. Mở file HTML để in.'
                    : 'Đã tải nhãn vận đơn.',
            );
        },
        onError: (error) => toast.error(getErrorMessage(error)),
